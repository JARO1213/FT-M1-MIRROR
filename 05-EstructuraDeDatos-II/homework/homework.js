...
LinkedList.prototype.remove = function (){
  ...
  if(!current){
    ...
    return current.value
     ...
     const newValue = current.next.value;
     current.next = null       
     return  newValue
   }
  }
...
HashTable.prototype.set = function(key, value){
  if(typeof key !== 'string'){
     throw TypeError('Keys must be a string')
  }
  // valor es igual al numBox....
  let valor =  this.hash(key);
  if(this.buckets[valor] === undefined){
    this.buckets[valor] = {[key]:value}
  }
  this.buckets[valor][key] = value
  return this.buckets;
}
HashTable.prototype.get = function(key){
  let valor = this.hash(key);
    if(this.buckets[valor][key]){
     return this.buckets[valor][key];
}
}
